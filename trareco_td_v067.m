...
function fname_fasta = trareco_td_v067( config_file )
...
Cvg_dep_wgt_div = round( cfg.nominal_read_length );
Edge_seg_len_min = cfg.mean_frag_length; 
E_cwgt = 0;
            ...
            if Group_Size(group_idx) <= 1
                ...
                if flag == 0
                    ...
                    for k = 1:1:n_paths
                        path_idx = k;
                        if ~isempty( path_mat(path_idx,:) )
                            if trc_len(path_idx) >= max( mx_trc_len*(cfg.min_rlen_percent/100), max( cfg.min_tr_length, cfg.mean_frag_length ) )
            ...
            if seg(idx_r).len == 0
                ...
                if n_out_deg == 0 && n_in_deg == 0
                    if seg(idx_r).len > mlen*2
                        ...
                        % ave_cvg_dep_eff(idx_r) = mean( sum( seg(idx_r).cvg_dep(:,1:seg(idx_r).len)));
                        ave_cvg_dep_eff(idx_r) = max( sum( seg(idx_r).cvg_dep(:,1:seg(idx_r).len)));
                        e_wgt(idx_r,1) = ceil( double(seg(idx_r).len)/Wgt_div );
                    end
                    if ave_cvg_dep_eff(idx_r) < 1
                        fprintf('\n      ERROR: cdep == 0, (%d: %f) ', idx_r, seg(idx_r).ave_cvg_dep );
...
if R_mode > 1
    ...
    if fp_t < 0
        ...
        fp_r1 = fopen( sprintf('%s_1.reads', out_file_prefix ), 'rt' );
        [type, fn2, ext2] = get_file_type( input_2 );
        fp_r2 = fopen( sprintf('%s_2.reads', out_file_prefix ), 'rt' );
...
N_tr_mpc = 0;
seg_m = seg(1:n_seg);
ave_cvg_dep_eff_m = ave_cvg_dep_eff;
clear seg;
clear seg_n;
...
for n = 1:1:N_groups
    ...
    if n_seg_selected <= 1
        idx = seg_idxs;
        if n_seg_selected == 1 % && seg_m(idx).len >= cfg.connection_threshold_short
            b_tmp = 0;
            if seg_m(idx).len < cfg.min_tr_length || seg_m(idx).ave_cvg_dep < 2 % cfg.min_cvg_depth
                b_tmp = 1;
            end
            if b_tmp == 0
                N_tr_cand = N_tr_cand + 1;
                % ave_cvg_dep_eff_m(idx) = seg_m(idx).ave_cvg_dep;
                if seg_m(idx).len > cfg.mean_frag_length
                    Abn = (ave_cvg_dep_eff_m(idx)); %.*(seg_m(idx).len/(seg_m(idx).len-cfg.mean_frag_length)); %cfg.mean_frag_length));
                else
                    Abn = (ave_cvg_dep_eff_m(idx));
                end
                tpm = 1000000000*Abn/N_bases;
                str_hdr = sprintf('>T_%d_%d_%d_%f_%f_%d\t%d\t%d\t%d\t%f\t%f\t%d\t%d\t%d', ...
                    n, 1, Group_Size(gidx), ave_cvg_dep_eff_m(idx), tpm, 100, ...
                    n, 1, seg_m(idx).len, (ave_cvg_dep_eff_m(idx)), tpm, N_reads_valid, N_bases, 100 );
                fprintf(fp_t,'%s\n', str_hdr);
                tr_seq_c = char(sub_NumSeq2NTstr( seg_m(idx).seq ));
                fprintf(fp_t,'%s\n', tr_seq_c);
                max_tr_len = max( max_tr_len, seg_m(idx).len );
                N_tr_single = N_tr_single + 1;
            end
            if ave_cvg_dep_eff_m(idx) < 1
                fprintf('\n      ERROR: cdep == 0, (%d: %f, %f) ', idx, seg_m(idx).ave_cvg_dep, ave_cvg_dep_eff_m(idx) );
            end
        end
        if mod( n, 20 ) == 0 || n == N_groups
            if Nchar > 0
       ...
       %% Check loop
        ...
        mn_cvg = round( min( [seg_m(seg_idxs).ave_cvg_dep] ) );
        if flag > 0
            % for k = 1:1:n_seg_selected
            xx = 0;
            while( 1 )
                [axx, si] = sort( [seg_m(seg_idxs).ave_cvg_dep], 'ascend' );
...
%                 end
                ...
                bsv( si(1:n_rm) ) = 0;
                mn_cvg = round(seg_m(seg_idxs(si(n_rm))).ave_cvg_dep);
                n_cnt = 0;
                ...
                for m = 1:1:n_grps
                    sit = sis( grp_idx == m );
                    sc_mat_tmp = seg_connection_mat_selected( sit, sit );
                    min_cvgs(m) = min( [ seg_m( seg_idxs( sit ) ).ave_cvg_dep ] );
...
%         end
%         Nchar = Nchar_save;
        seg = repmat( sg, n_seg_selected, 1 );
        ave_cvg_dep_eff = zeros( n_seg_selected, 1 );
        for k = 1:1:n_seg_selected
            seg(k) = seg_m(seg_idxs(k));
            ave_cvg_dep_eff(k) = ave_cvg_dep_eff_m(seg_idxs(k));
        end       
        
        if( 0 )
            sidxs = 1:1:n_seg_selected;
            % Get edge weights
            edges = find( seg_connection_mat_selected );
            num_edges = length(edges);
            num_eqs = sum( sum( seg_connection_mat_selected ) > 0 ) + sum( sum( seg_connection_mat_selected, 2 ) > 0 );
            ...
            for m = 1:1:n_seg_selected
                ...
                if sum( seg_connection_mat_selected(m,:) ) > 0
                    e_cnt = e_cnt + 1;
                    y(e_cnt) = sum( seg(sidxs(m)).cvg_dep(:,end) ); % ave_cvg_dep_eff(m); % 
                    Ts(e_cnt, en_mat(m, en_mat(m,:) > 0 ) ) = 1;
                end
                % for each column, incoming edges, corresponding segment start
                if sum( seg_connection_mat_selected(:,m) ) > 0
                    e_cnt = e_cnt + 1;
                    y(e_cnt) = sum( seg(sidxs(m)).cvg_dep(:,1) ); % ave_cvg_dep_eff(m); % 
                    Ts(e_cnt, en_mat( en_mat(:,m) > 0, m ) ) = 1;
                end
            end
            Step_size = cfg.lasso_step_size; 
            ...
            seg_n = repmat( sg, n_seg_selected+1000, 1 );
            seg_n(1:n_seg_selected) = seg;
            ave_cvg_dep_eff_n = zeros( n_seg_selected+1000, 1 );
            ave_cvg_dep_eff_n(1:n_seg_selected) = ave_cvg_dep_eff;
            
            n_seg_selected_new = n_seg_selected;
            sidxs = 1:1:n_seg_selected;
            for k = 1:n_seg_selected
                idx_r = sidxs(k); 
                seg(idx_r).seq = f03_seq_est( seg(idx_r).cvg_dep );
                ...
                in_nodes = find( seg_connection_mat_selected(:,k) );
                indeg = length( in_nodes );
                out_nodes = find( seg_connection_mat_selected(k,:) );
                outdeg = length( out_nodes );
                
                if indeg > 0 && outdeg > 0 
                    
                    wgt = 0;
                    for m1 = 1:length(in_nodes)
                        wgt = wgt + edge_wgt_vec( en_mat( in_nodes(m1), idx_r ) );
                    end
                    wgt_in = wgt;
                    wgt = 0;
                    for m1 = 1:length(out_nodes)
                        wgt = wgt + edge_wgt_vec( en_mat( idx_r, out_nodes(m1) ) );
                    end
                    wgt_out = wgt;
                    r = wgt_in/wgt_out;
                    if r >= 1/min(wgt_in,wgt_out) + 1.4
                            seg_mat_tmp = zeros( n_seg_selected_new+1 );
                            seg_mat_tmp( 1:n_seg_selected_new, 1:n_seg_selected_new ) = seg_connection_mat_selected;
                            seg_connection_mat_selected = seg_mat_tmp;
                            
                            n_seg_selected_new = n_seg_selected_new + 1;
                            idx = n_seg_selected_new;
                            seg_n(idx).len = 1;
                            cdt = zeros(4,1);
                            cdt( 1 ) = round(abs(wgt_in - wgt_out)); 
                            seg_n(idx).cvg_dep = cdt;
                            seg_n(idx).seq = f03_seq_est( seg_n(idx).cvg_dep );
                            seg_n(idx).ave_cvg_dep = round(( sum( seg_n(idx).cvg_dep )) );
                            ave_cvg_dep_eff(idx) = seg_n(idx).ave_cvg_dep;
                            
                            seg_connection_mat_selected(idx_r,n_seg_selected_new) = 1;
                            sidxs = 1:1:n_seg_selected_new;
                    else
                        if 1/r >= 1/min(wgt_in,wgt_out) + 1.4
                            seg_mat_tmp = zeros( n_seg_selected_new+1 );
                            seg_mat_tmp( 1:n_seg_selected_new, 1:n_seg_selected_new ) = seg_connection_mat_selected;
                            seg_connection_mat_selected = seg_mat_tmp;
                            ...
                            seg_connection_mat_selected(n_seg_selected_new,idx_r) = 1;
                            sidxs = 1:1:n_seg_selected_new;
                        end
                    end
                end
            end
            % output: n_seg_selected_new, seg_connection_mat_selected, 
            n_seg_selected = n_seg_selected_new; 
            seg = seg_n( 1:n_seg_selected );
        end
        
        [n_grps, grp_idx] = f04_group_search_v01a( seg_connection_mat_selected );
        seg_connection_mat_selected_save = seg_connection_mat_selected;
        seg_idxs_save = 1:1:n_seg_selected; % seg_idxs;
        ...
        for kg = 1:1:n_grps
            ...
            if n_seg_selected == 1
                ...
                idx = seg_idxs(1);
                b_tmp = 0;
                if seg(idx).len < cfg.min_tr_length || seg(idx).ave_cvg_dep < 2 %cfg.min_cvg_depth
                    b_tmp = 1;
                end
                if b_tmp == 0
                    N_tr_cand = N_tr_cand + 1;
                    % ave_cvg_dep_eff(idx) = seg(idx).ave_cvg_dep;
                    if seg(idx).len > cfg.mean_frag_length
                        Abn = (ave_cvg_dep_eff(idx)); %.*(seg(idx).len/(seg(idx).len-cfg.mean_frag_length)); %cfg.mean_frag_length));
                    else
                        Abn = (ave_cvg_dep_eff(idx));
                    end
                    tpm = 1000000000*Abn/N_bases;
                    str_hdr = sprintf('>T_%d_%d_%d_%f_%f_%d\t%d\t%d\t%d\t%f\t%f\t%d\t%d\t%d', ...
                        n, 1, Group_Size(gidx), ave_cvg_dep_eff(idx), tpm, 100, ...
                        n, 1, seg(idx).len, (ave_cvg_dep_eff(idx)), tpm, N_reads_valid, N_bases, 100 );
                    fprintf(fp_t,'%s\n', str_hdr);
                ...
                edge_compat_mat = zeros( num_edges );
                
                [n_paths, path_mat, path_len, flag, nchar] = f04_path_search_v02a( seg_connection_mat_selected, n_max_paths );
                Nchar = Nchar + nchar;
                
                if (R_mode > 1) && (max(path_len(1:n_paths)) > 2) % (n_paths >= 60) % && (b_all > 0) 
                    
                    % INIT edge_compat_mat
                    for k = 1:n_paths
                        if path_len(k) > 2
                            for m = 3:path_len(k)
                                p1 = path_mat(k,m-2);
                                p2 = path_mat(k,m-1);
                                p3 = path_mat(k,m);
                                edge_compat_mat( en_mat(p1,p2), en_mat(p2,p3) ) = 1;
                            end
                        end
                    end
                
                    idx = seg_idxs(1);
                    gidx_org = seg(idx).group_id_org;
                    z_tmp = 0;
                    for k = 2:1:n_seg_selected
                        idx = seg_idxs(k);
                        if seg(idx).group_id_org ~= gidx_org
                            z_tmp = 1;
                            break;
                        end
                    end
                    if z_tmp > 0 
                        fprintf('      ERROR: seg(idx).group_id_org ~= gidx_org \n');
                    end

                    b_cand_valid = ones( n_paths, 1 );
                    Tr_cand_tmp.len = uint32(0);
                    Tr_cand_tmp.seq = int8(0);
                    Tr_cand_tmp.niv = 0;
                    Tr_cand_tmp.rcv = uint32(0);
                    Tr_cand_cur = repmat( Tr_cand_tmp, n_paths, 1 );
                    for k = 1:1:n_paths
                        path_idx = k;
                        idx = seg_idxs( path_mat(path_idx,1) );
                        Tr_seq_tmp = seg(idx).seq;
                        Tr_vec_tmp = ones(1,seg(idx).len).*double(path_mat(path_idx,1));
                        for m = 2:1:path_len(path_idx)
                            idx = seg_idxs( path_mat(path_idx,m) );
                            if seg(idx).len > 0
                                Tr_seq_tmp = [Tr_seq_tmp seg(idx).seq];
                                Tr_vec_tmp = [Tr_vec_tmp ones(1,seg(idx).len).*double(path_mat(path_idx,m))];
                            end
                        end
                        Tr_cand_cur(k).seq = Tr_seq_tmp;
                        Tr_cand_cur(k).len = length( Tr_seq_tmp );
                        Tr_cand_cur(k).niv = Tr_vec_tmp;
                        Tr_cand_cur(k).rcv = zeros(1,Tr_cand_cur(k).len,'uint32');
                    end

                    num_N_mer = 8;  
                    seg_sel_dsr = 8;
                    Normalized_Dist_threshold = cfg.norm_dist_threshold; 
                    mlen = cfg.nominal_read_length*2;

                    Seg_Nmer_map = zeros( n_paths, 4^num_N_mer, 'int8' );
                    for k = 1:n_paths
                        [seg_seq_Nmer1, sl1] = sub_NumSeq2NmerSeq( Tr_cand_cur(k).seq, num_N_mer );
                        [seg_seq_Nmer2, sl2] = sub_NumSeq2NmerSeq( 3-Tr_cand_cur(k).seq(end:-1:1), num_N_mer );
                        Seg_Nmer_map( k, [seg_seq_Nmer1(1:sl1) seg_seq_Nmer2(1:sl2)]+1 ) = 1;
                    end

                    rdm_idxs = find( (read_group_map(:,2) == gidx_org) + (read_group_map(:,3) == gidx_org) );
                    nchar = fprintf('(%d,%d,%d)', n_paths, length(rdm_idxs), round( mean([Tr_cand_cur(1:end).len]) ) );
%                     Nchar = Nchar + nchar;
                    n_step = round( length(rdm_idxs)/10 );
                    n_aligned = 0;
                    n_tested = 0;
                    for k = 1:1:length(rdm_idxs)
                        rd_idx = read_group_map( rdm_idxs(k), 1 );
                        len1 = length(rds(rd_idx).seq1);
                        len2 = length(rds(rd_idx).seq2);
                        % if (len1 > 24) || (len2 > 24)
                        if (read_group_map( rdm_idxs(k), 2 ) == gidx_org) && (read_group_map( rdm_idxs(k), 3 ) == gidx_org)
                            n_tested = n_tested + 1;
                            [seg_seq_Nmer1, sl1] = sub_NumSeq2NmerSeq( rds(rd_idx).seq1, num_N_mer );
                            [seg_seq_Nmer2, sl2] = sub_NumSeq2NmerSeq( rds(rd_idx).seq2, num_N_mer );

                            Ave_num_error_normalized = ceil( max(len1,len2)*Normalized_Dist_threshold );
                            Match_threshold1 = round( max( (len1 - (num_N_mer-seg_sel_dsr))/seg_sel_dsr - Ave_num_error_normalized*(num_N_mer/seg_sel_dsr), 2 ) );
                            Match_threshold2 = round( max( (len2 - (num_N_mer-seg_sel_dsr))/seg_sel_dsr - Ave_num_error_normalized*(num_N_mer/seg_sel_dsr), 2 ) );

                            tpis = find( b_cand_valid );
                            Nmer_Matches1 = sum( Seg_Nmer_map( tpis, seg_seq_Nmer1(1:seg_sel_dsr:end)+1 ), 2 );
                            Nmer_Matches2 = sum( Seg_Nmer_map( tpis, seg_seq_Nmer2(1:seg_sel_dsr:end)+1 ), 2 );
                            spis = find( (Nmer_Matches1 >= Match_threshold1).*(Nmer_Matches2 >= Match_threshold2) );

                            b_tmp = 0;
                            for m = 1:1:length(spis)
                                path_idx = tpis(spis(m));
                                if Tr_cand_cur(path_idx).len > max(len1,len2)
                                    [pos(1), ref_seq_idx1, dst1] = sub_tco( Tr_cand_cur(path_idx).seq, rds(rd_idx).seq1, Normalized_Dist_threshold, cfg.bool_ss_ind );
                                    [pos(2), ref_seq_idx2, dst2] = sub_tco( Tr_cand_cur(path_idx).seq, rds(rd_idx).seq2, Normalized_Dist_threshold, cfg.bool_ss_ind );
                                    if pos(1) ~= 0 && pos(2) ~= 0
                                        pos(3) = abs(pos(1)) + length(rds(rd_idx).seq1)-1;
                                        pos(4) = abs(pos(2)) + length(rds(rd_idx).seq2)-1;
                                        sp = min( abs(pos) );
                                        ep = max( abs(pos) );
                                        ...
                                        if sp < ep && ep <= Tr_cand_cur(path_idx).len
                                            c_cnt = 0;
                                            ni_p = Tr_cand_cur(path_idx).niv(sp);
                                            ei_p = 0;
                                            for m1 = sp+1:ep
                                                if Tr_cand_cur(path_idx).niv(m1) ~= ni_p
                                                    c_cnt = c_cnt + 1;
                                                    if c_cnt == 1
                                                        p1 = Tr_cand_cur(path_idx).niv(m1-1);
                                                        p2 = Tr_cand_cur(path_idx).niv(m1);
                                                        ei_p = en_mat(p1,p2);
                                                    else
                                                        p1 = Tr_cand_cur(path_idx).niv(m1-1);
                                                        p2 = Tr_cand_cur(path_idx).niv(m1);
                                                        ei_c = en_mat(p1,p2);
                                                        edge_compat_mat( ei_p, ei_c ) = edge_compat_mat( ei_p, ei_c ) + 1;
                                                        ei_p = ei_c;
                                                    end
                                                    ni_p = Tr_cand_cur(path_idx).niv(m1);
                                                end
                                            end
                                        end
                                        b_tmp = 1;
                            ...
                            n_aligned = n_aligned + b_tmp;
                        else
                            if (read_group_map( rdm_idxs(k), 2 ) == gidx_org)
                                n_tested = n_tested + 1;
                                [seg_seq_Nmer1, sl1] = sub_NumSeq2NmerSeq( rds(rd_idx).seq1, num_N_mer );
                                Ave_num_error_normalized = ceil( len1*Normalized_Dist_threshold );
                                Match_threshold1 = round( max( (len1 - (num_N_mer-seg_sel_dsr))/seg_sel_dsr - Ave_num_error_normalized*(num_N_mer/seg_sel_dsr), 2 ) );
                                tpis = find( b_cand_valid );
                                Nmer_Matches1 = sum( Seg_Nmer_map( tpis, seg_seq_Nmer1(1:seg_sel_dsr:end)+1 ), 2 );
                                spis = find( (Nmer_Matches1 >= Match_threshold1) );
                                b_tmp = 0;
                                for m = 1:1:length(spis)
                                    path_idx = tpis(spis(m));
                                    if Tr_cand_cur(path_idx).len > len1
                                        [pos(1), ref_seq_idx1, dst1] = sub_tco( Tr_cand_cur(path_idx).seq, rds(rd_idx).seq1, Normalized_Dist_threshold, cfg.bool_ss_ind );
                                        if pos(1) ~= 0 
                                            pos(2) = abs(pos(1)) + length(rds(rd_idx).seq1);
                                            sp = min( abs(pos(1:2)) );
                                            ep = max( abs(pos(1:2)) );
                                            if sp < ep && ep <= Tr_cand_cur(path_idx).len
                                                c_cnt = 0;
                            ...
                            % end
                                ...
                                if (read_group_map( rdm_idxs(k), 3 ) == gidx_org)
                                    n_tested = n_tested + 1;
                                    [seg_seq_Nmer1, sl1] = sub_NumSeq2NmerSeq( rds(rd_idx).seq2, num_N_mer );
                                    Ave_num_error_normalized = ceil( len2*Normalized_Dist_threshold );
                                    Match_threshold1 = round( max( (len2 - (num_N_mer-seg_sel_dsr))/seg_sel_dsr - Ave_num_error_normalized*(num_N_mer/seg_sel_dsr), 2 ) );
                                    tpis = find( b_cand_valid );
                                    Nmer_Matches1 = sum( Seg_Nmer_map( tpis, seg_seq_Nmer1(1:seg_sel_dsr:end)+1 ), 2 );
                                    spis = find( (Nmer_Matches1 >= Match_threshold1) );
                                    b_tmp = 0;
                                    for m = 1:1:length(spis)
                                        path_idx = tpis(spis(m));
                                        if Tr_cand_cur(path_idx).len > len2
                                            [pos(1), ref_seq_idx1, dst1] = sub_tco( Tr_cand_cur(path_idx).seq, rds(rd_idx).seq2, Normalized_Dist_threshold, cfg.bool_ss_ind );
                                            if pos(1) ~= 0 
                                                pos(2) = abs(pos(1)) + length(rds(rd_idx).seq2);
                                                sp = min( abs(pos(1:2)) );
                                                ep = max( abs(pos(1:2)) );
                                                if sp < ep && ep <= Tr_cand_cur(path_idx).len
                                                    c_cnt = 0;
                    ...
                    % UPDATE edge_compat_mat
                    u_cnt = 0;
                    for k = 1:num_edges
                        eis = find( edge_compat_mat(k,:) > 1 );
                        if ~isempty(eis)
                            for m1 = 1:num_edges
                                if edge_compat_mat(k,m1) == 1
                                    edge_compat_mat(k,m1) = 0;
                                    u_cnt = u_cnt + 1;
                                end
                            end
                        end
                        ...
                        for k = 1:n_paths
                            if path_len(k) > 2
                                for m = 3:path_len(k)
                                    ...
                                    p2 = path_mat(k,m-1);
                                    p3 = path_mat(k,m);
                                    if edge_compat_mat( en_mat(p1,p2), en_mat(p2,p3) ) == 0
                                        b_cand_valid(k) = 0;
                                        break;
                                    end
                                end
                    ...
                    N_count = N_count + length(rdm_idxs);
                    N_aligned = N_aligned + n_aligned;
                    
                    if sum( b_cand_valid ) ~= length( b_cand_valid ) 
                        n_paths_new = 0;
                        for k = 1:1:n_paths
                            if b_cand_valid(k) > 0
                                n_paths_new = n_paths_new + 1;
                                if k ~= n_paths_new
                                    path_len(n_paths_new) = path_len(k);
                                    path_mat(n_paths_new,:) = path_mat(k,:);
                                end
                            end
                        end
                        n_paths = n_paths_new;
                        path_len = path_len(1:n_paths);
                        path_mat = path_mat(1:n_paths,:);
                        
                        nchar = fprintf('(%d, %d, %d) \n', sum(b_cand_valid), round(n_aligned), (n_tested) );
                        Nchar = 0; %Nchar + nchar;
                    else
                        nchar = fprintf('(%d, %d, %d) \n', n_paths, round(n_aligned), (n_tested) );
                        Nchar = 0; %Nchar + nchar;
                    end
                end
...
%                     end
                    ...
                    for k = 1:1:cset_size(midx)
                        if b_tr_valid(k) > 0
                           if trs(k).len > cfg.mean_frag_length 
                                Abundance(k) = trs(k).abn.*(trs(k).len/(trs(k).len-cfg.mean_frag_length)); 
                            else
                                Abundance(k) = trs(k).abn; 
                            end
                            Abn_sum = Abn_sum + Abundance(k);
                        end
                    end
                    for k = 1:1:cset_size(midx)
                        if b_tr_valid(k) > 0
                            ...
                            if trs(k).len > cfg.mean_frag_length
                                ...
                                Abundance(k) = trs(k).abn; %.*(trs(k).len);
                            end
                            b_tmp = 0;
                            if b_tmp == 0 && length(Tr_seq_tmp) >= cfg.connection_threshold_short
                                ...
                                if b_tmp == 0
                                    ...
                                    str_hdr = sprintf('>I_%d_%d_%d_%f_%f_%d\t%d\t%d\t%d\t%f\t%f\t%d\t%d\t%d', ...
                                        n, k, cset_size(midx), Abundance(k), tpm, round(100*Abundance(k)/Abn_sum), ...
                                        n, k, length(Tr_seq_tmp), Abundance(k), tpm, N_reads_valid, N_bases, round(100*Abundance(k)/Abn_sum) );
                                    fprintf(fp_t,'%s\n', str_hdr);
