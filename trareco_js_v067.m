...
function fname_out = trareco_js_v067( config_file, soption ) 
...
% cfg_val = [cfg.norm_dist_threshold cfg.connection_threshold_short round(cfg.max_overlap_depth*3) cfg.connection_threshold cfg.bool_ss_ind];
...
for k = 1:1:n_seg
    cstr = textscan(fp_t,'%s',1, 'Delimiter', '\n', 'BufSize', 1e6 );
    if isempty(cstr{1}) 
        break;
    else
        linea = cstr{1}{:};
...
%     linef = fgets(fp_t);
    if linef < 0
        ...
        if linea(1) == '>'
            ...
            seg(n_cnt).len = length(lineb);
            % seg(n_cnt).seq = zeros( 1, seg(n_cnt).len + 1000;
            seg(n_cnt).seq = sub_NTstr2NumSeq( lineb(1:end) );
...
%     str_disp = sprintf('   Removing contigs with its volume below min value '); %, cfg.min_cvg_depth_js );
%     fprintf( '\n%s', str_disp );
%     fprintf( fp_log, '\n%s', str_disp );
% 
%     seg_cnt = 0;
...
% 
%         if seg(n).ave_cvg_dep*seg(n).len > cfg.nominal_read_length*2 
%             seg_cnt = seg_cnt + 1;
%             if seg_cnt ~= n
%                 seg(seg_cnt).id = seg(n).id;
%                 seg(seg_cnt).len = seg(n).len;
...
%% pre-junction search
    ...
    cfg_val = [cfg.norm_dist_threshold cfg.connection_threshold_short round(cfg.max_overlap_depth*3) cfg.connection_threshold_short cfg.bool_ss_ind];
    % s_idxs = find( [jpc(1:end).nj] > 0 );
...
for lp = 1:1:N_Loops
    ...
    lens = [seg(1:n_seg).len];
    seg_idxs_tmp = find( lens > cfg.short_seg_threshold );
    for n = 1:1:length(seg_idxs_tmp) % n_seg
        dividend_idx = seg_idxs_tmp(n); 
...
%                         end
                ...
                % if mod(n, n_step) == 0
                    ...
                    Nchar = fprintf( '%s %d/%d/%d (%d/%d)', str_hdr, n, length(seg_idxs_tmp), round(n_seg_sel), lp, N_Loops );                     
                % end
                clear j_cnt_divisor;
                clear j_idx_divisor;
            end
...
tic
    ...
    jidxs = find( junction_info(1:N_junctions,13) );
    for km = 1:length(jidxs)
        n = jidxs(km);
            ...
            if group_id( junction_info(n,2) ) == 0
                ...
                if group_id( junction_info(n,3) ) == 0
                    ...
                    if group_id( junction_info(n,3) ) == group_id( junction_info(n,2) )
                        ...
                        junction_info( junction_info(1:n-1,1) == gid_tmp ) = junction_info(n,1);
                        group_id( group_id(1:n_seg) == gid_tmp ) = junction_info(n,1);
                    end
                end
            end
        end
    ...
    sidxs = find( group_id(1:n_seg) == 0 );
    if ~isempty( sidxs )
        gidx_tmp = find( b_group_valid, length(sidxs) );
        group_id(sidxs) = gidx_tmp;
        b_group_valid( gidx_tmp ) = 0;
    end
...
% end
% 
if N_groups == sum( 1 - b_group_valid )
    Group_Size = zeros(N_groups,1);
    for k = 1:1:n_seg
        m = group_id(k);
        Group_Size(m) = Group_Size(m) + 1;
            ...
            if group_id( junction_info(n,2) ) == 0
                ...
                if group_id( junction_info(n,3) ) == 0
                    ...
                    if group_id( junction_info(n,3) ) == group_id( junction_info(n,2) )
                        ...
                        gid_tmp_n = min( group_id( junction_info(n,2) ), group_id( junction_info(n,3) ) );
                        gid_tmp_x = max( group_id( junction_info(n,2) ), group_id( junction_info(n,3) ) );
                        junction_info( junction_info(1:n-1,1) == gid_tmp_x, 1 ) = gid_tmp_n;
                        junction_info(n,1) = gid_tmp_n; %group_id( junction_info(n,2) );
                        group_id( group_id(1:n_seg) == gid_tmp_x ) = gid_tmp_n; %junction_info(n,1);
                        b_group_valid( gid_tmp_x ) = 1;
                    end
                end
            end
        end
...
while(1)
    ...
    if R_mode >= 0
        if max( Group_Size ) <= cfg.max_n_cntg_per_group
            ...
            for k = 1:1:length(gidx)
                jidx = find( (junction_info(:,1) == gidx(k)).*(junction_info(:,13) > 0) );
                if ~isempty(jidx)
    ...
    n_loop =  n_loop + 1;   
    % regroup
    gidx_all = find( Group_Size > cfg.max_n_cntg_per_group );
    for km = 1:1:length(gidx_all)
        gidx_cur = gidx_all(km);
        b_group_valid( gidx_cur ) = 0;
        s_idxs_tmp = find( group_id == gidx_cur );
        group_id( s_idxs_tmp ) = 0;
        jidx_cur = find( (junction_info(:,1) == gidx_cur).*(junction_info(:,13) > 0) );
        junction_info_tmp = junction_info( jidx_cur, : );
        % group_id( junction_info_tmp(:,2) ) = 0;
        % group_id( junction_info_tmp(:,3) ) = 0;

        for n = 1:length(jidx_cur)
            % k = jidx_cur(n);
            if junction_info_tmp(n,13) > 0
                % ndd( junction_info_tmp(n,2), 1 ) = ndd( junction_info_tmp(n,2), 1 ) + 1;
                % ndd( junction_info_tmp(n,3), 2 ) = ndd( junction_info_tmp(n,3), 2 ) + 1;
                if group_id( junction_info_tmp(n,2) ) == 0
                    if group_id( junction_info_tmp(n,3) ) == 0
                        N_groups_tmp = find( b_group_valid, 1 );
                        b_group_valid( N_groups_tmp ) = 0;
                        group_id( junction_info_tmp(n,2) ) = N_groups_tmp;
                        group_id( junction_info_tmp(n,3) ) = N_groups_tmp;
                        junction_info_tmp(n,1) = N_groups_tmp;
                    else
                        group_id( junction_info_tmp(n,2) ) = group_id( junction_info_tmp(n,3) );
                        junction_info_tmp(n,1) = group_id( junction_info_tmp(n,3) );
                    end
                else
                    if group_id( junction_info_tmp(n,3) ) == 0
                        junction_info_tmp(n,1) = group_id( junction_info_tmp(n,2) );
                        group_id( junction_info_tmp(n,3) ) = junction_info_tmp(n,1);
                    else
                        % ERROR
                        if group_id( junction_info_tmp(n,3) ) == group_id( junction_info_tmp(n,2) )
                            junction_info_tmp(n,1) = group_id( junction_info_tmp(n,2) );
                        else
                            gid_tmp_n = min( group_id( junction_info_tmp(n,2) ), group_id( junction_info_tmp(n,3) ) );
                            gid_tmp_x = max( group_id( junction_info_tmp(n,2) ), group_id( junction_info_tmp(n,3) ) );
                            junction_info_tmp( junction_info_tmp(1:n-1,1) == gid_tmp_x, 1 ) = gid_tmp_n;
                            junction_info_tmp(n,1) = gid_tmp_n; %group_id( junction_info(n,2) );
                            group_id( group_id(1:n_seg) == gid_tmp_x ) = gid_tmp_n; %junction_info(n,1);
                            b_group_valid( gid_tmp_x ) = 1;
%                             
%                             junction_info_tmp(n,1) = group_id( junction_info_tmp(n,2) );
%                             gid_tmp = group_id( junction_info_tmp(n,3) );
%                             junction_info_tmp( junction_info_tmp(1:n-1,1) == gid_tmp ) = junction_info_tmp(n,1);
%                             group_id( group_id(s_idxs_tmp) == gid_tmp ) = junction_info_tmp(n,1);
                        end
                    end
                end
            end
        end
        junction_info( jidx_cur, : ) = junction_info_tmp;
    end
    ...
    N_groups = max( group_id ); %sum( 1 - b_group_valid );
    Group_Size = zeros(N_groups,1);
    for k = 1:1:n_seg
        m = group_id(k);
        Group_Size(m) = Group_Size(m) + 1;
...
%     if N_groups ~= sum( 1 - b_group_valid )
%         fprintf('      ERROR: N_groups ~= sum( 1 - b_group_valid ) ...            ');
%     end
end
% toc
% fprintf('                           ');
...
fprintf( '%s', str_disp ); 
fprintf( fp_log, '%s', str_disp ); 
str_disp = sprintf(' Group check (%d, %d) ', N_groups, max(group_id) );
fprintf( '%s', str_disp ); 
...
str_disp = sprintf(' (%d, %d) ', Nrt, Nrt_mapped );
fprintf( '%s', str_disp );
fprintf( fp_log, '%s', str_disp );
...
function numRows = linecount(filename)
fid = fopen(filename, 'rt');
   chunksize = 1e7; % read chuncks of 1MB at a time
   numRows = 1;
   while ~feof(fid)
